...
try:
    ...
    for groupInx in range(0,len(rawCanDataResponse),16):
        ...
        try:
            allDtcCollection.Add(dtc+statusByte, statusByte)
        except:
            TestAppLogger.Info("DTC Parsing Python Script:  Error adding DTC to allDtcCollection.  Either a duplicate DTC or not running TPass version 1.5.0.14 or greater.  {0}", dtc+statusByte)
