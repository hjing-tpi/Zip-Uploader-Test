#Create a Yanfeng Test Results Record and format and return the Printed Label Text for TPass to send to a printer
...
numberFaultsAdded = 0
#84964072;PASS
#84971421;FAIL;IPC PART NUMBER,........PN from Broadcast: 42786792 ,........No PN from Device,RADIO EXTENDED DIAGNOSTIC,TEST ABORTED
...
def SetProcessData(reportId, reportData):
    ...
    if (reportId and reportData):
        # Saving Process Data is not implemented for Yanfeng
        processResultsText = ""
...
try:
    ...
    try:
        ...
        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Test Results Processing Script:  Error Reading Test ID File, will default to Tester ID = 0.  Exception Occurred = " + str(inst), True, True)
    ...
    printedLabelText = printedLabelText + "Job ID:" + productIdentification.SecondaryId + "\r\n"
    ...
    # Set Results to send to Yanfeng
    ...
    MainTPassScripting.InterfaceUiLogger("TCPInterface", "Error Creating Results Record.  Exception Occurred = " + str(inst), True, True)
    isSuccess = False

# Create Results file for SQL connect to process
try:
    try:
        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Test Results - " + testResultsText, False, False)
        File.WriteAllText(resultsFile + "TMP", testResultsText)
        File.Move(resultsFile + "TMP", resultsFile)
        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Results File Written - " + resultsFile, False, False)
    except Exception as inst:
