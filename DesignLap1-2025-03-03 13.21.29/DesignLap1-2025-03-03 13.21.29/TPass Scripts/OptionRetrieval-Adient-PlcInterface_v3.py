...
try:
    ...
    # If data didn't come from File drop, retrieve the build data using PLCInterface.  Otherwise data received is complete
    # if str(MainTPassScripting.StartTestCycleTriggerType) != str(MainTPassScripting.TriggerType.ManualEntry):
        ...
        # # Create Request file for PLCInterface to read, query the plant database and write the results
        # try:
            # File.WriteAllText(requestFile + "TMP", RunTestCycleId)
            # File.Move(requestFile + "TMP", requestFile)
            # MainTPassScripting.InterfaceUiLogger("PLCInterface", "Request File Written - " + requestFile, False, False)
        # except Exception as inst:
        ...
        # try:
            ...
            # while (DateTime.Now - startDateTime).TotalMilliseconds < buildDataRequestTimeoutMsec and not buildDataFileFound:
                ...
                # if File.Exists(buildDataFile):
                    ...
                    # MainTPassScripting.InterfaceUiLogger("PLCInterface", "Build Data File Received - " + buildDataFile, False, False)

                    # #Read contents and delete file
                    # productBuildData = File.ReadAllText(buildDataFile)
                    # MainTPassScripting.InterfaceUiLogger("PLCInterface", "Build Data Received - " + productBuildData, False, False)
                    # isSuccess = True
            ...
            # if buildDataFileFound == False:
                # MainTPassScripting.InterfaceUiLogger("PLCInterface", "Build Data File Not Received - " + buildDataFile, True, False)
                # productBuildData = ""
        
        # except Exception as inst:
            # MainTPassScripting.InterfaceUiLogger("PLCInterface", "Read Build Data File Exception Occurred" + str(inst), True, True)
...
#   Date: 09192022
#   Version: 3.0
#   Change: Change for manual entry of job number
#   Date: 09192022
#   Version: 2.0
#   Change: Create temporary request.txt file then rename, to avoid file contention issues with PLCInterface
#   Date: 05162022
#   Version: 1.0
