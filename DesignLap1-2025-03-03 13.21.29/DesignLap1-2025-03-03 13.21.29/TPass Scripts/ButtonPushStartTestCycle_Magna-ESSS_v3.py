...
try:
        ...
        #Get Password
        pwInput = MainTPassScripting.GetInputKeyBoard("Password", True)
        if pwInput != "":
            if pwInput.ToUpper() != password:
                # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                MainTPassScripting.InterfaceUiLogger("Magna-ESSS", "Invalid Password", True, True)
            else:
                input = MainTPassScripting.GetInputKeyPad("Recipe")
                if input != "":
                    ...
                    isSuccess = True
                    TPassLogger.Info("GetInputKeyPad() No Recipe Entered by Operator")
        else:
            isSuccess = True
