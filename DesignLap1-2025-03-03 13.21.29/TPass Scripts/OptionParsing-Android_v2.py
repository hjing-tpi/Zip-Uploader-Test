...
try:
    ...
    if str(MainTPassScripting.StartTestCycleTriggerType) == str(MainTPassScripting.TriggerType.ManualEntry):
        ...
        MainTPassScripting.InterfaceUiLogger("Serial Scan", "Option Parsing Script:  Product Build Data Scanned = " + BuildData, False, False)
        if BuildData == "":
            MainTPassScripting.InterfaceUiLogger("Serial Scan", "Option Parsing Script:  No Product Build Data in Scan", True, True)
            ...
            if len(buildDataFields) >= numberOfFieldsRequired and (len(buildDataFields) % 2) != 0:      #must be odd in length
                ...
                tertiaryId = buildDataFields[2]
                
                #Populate Option Codes found in broadcasted build data
                offset = -1
                for parameter in buildDataFields:
                    offset += 1
                    if (offset < 3):        #First part number is at field offset 3
                        continue
                    if (offset % 2) != 0:   #Odd offset which is the part number to check for NO-PART, then the next index is the options code
                        if ("NO-PART" in parameter):
                            optionName = buildDataFields[offset+1]
                            TPassLogger.Info("Option Parsing Script: Option not added.  NO-PART for Option Code ({0}) found in build data", optionName)
                            MainTPassScripting.InterfaceUiLogger("Serial Scan", "Option Parsing Script:  Option not added.  NO-PART for Option Code - " + optionName, False, False)
                            continue
                        else:
                            optionName = buildDataFields[offset+1]
                            ...
                            MainTPassScripting.InterfaceUiLogger("Serial Scan", "Option Parsing Script:  Added Configured Option Code - " + optionName, False, False)
            else:
                ...
                # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                MainTPassScripting.InterfaceUiLogger("Serial Scan", "Option Parsing Script:  Invalid number of fields for Product Build Data.  Must be at least " + str(numberOfFieldsRequired) + " fields and odd in length", True, True)
                isSuccess = False
