#Create a MERS Test Results Record, GEPICS Shipping Control Record and GSIP Quality Record using the DEMO interface
...
#   in  - COM object "gmEdiBuildResultsRecord" - DEMO Interface pointer for creating GEPICS Shipping Control records, GSIP Quality Records and MERS Test Results records
...
def SetFault(faultId, faultDescription):
    ...
    if (faultId):
        try:
            ...
            MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetFault() Fault Id Invalid and not saved.  Valid range is 0-65535.  Fault Id = " + faultId, True, True)
        else:
            if (ediFaultId >= 0 and ediFaultId <= 65535):
                try:
                    gmEdiBuildResultsRecord.HrSetFault(ediFaultId)
                    # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetFault() Fault =  " + faultId, False, False)
                except Exception as inst:
                    TPassLogger.Warn("Test Results Processing Script:  Error HrSetFault().  Exception Occurred :{0}", inst)
                    MainTPassScripting.InterfaceUiLogger("DEMO", "Error HrSetFault().  Exception Occurred = " + str(inst), True, False)
            else:
                TPassLogger.Warn("Test Results Processing Script:  Fault Id Invalid and not saved.  Valid range is 0-65535.  Fault Id = {0}", faultId)
                # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetFault() Fault Id Invalid and not saved.  Valid range is 0-65535.  Fault Id = " + faultId, True, True)
...
def SetProcessData(userValueId, userValue):
    if (userValueId and userValue):
        try:
            ...
            # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
            MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetUserValue() User Value Id Invalid and not saved.  Valid range is 1 - 413.  User Value Id = " + userValueId + ", Value = " + userValue, True, True)
        else:
            try:
                if (ediUserValueId >= 1 and ediUserValueId <= 413):
                    ...
                    if (ediUserValue >= -32768 and ediUserValue <= 32767):
                        ...
                        # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                        MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetUserValue() User Value Id = " + userValueId + ", Value = " + userValue, False, False)
                    else:
                        ...
                        # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                        MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetUserValue() User Value Invalid and not saved.  Valid range is -32,768 to 32,767.  User Value Id = " + userValueId + ", Value = " + userValue, True, True)
                else:
                    TPassLogger.Warn("Test Results Processing Script:  User Value Id Invalid and not saved.  Valid range is 1 - 413.  User Value Id = {0}, Value = {1}", userValueId, userValue)
                    # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
                    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetUserValue() User Value Id Invalid and not saved.  Valid range is 1 - 413.  User Value Id = " + userValueId + ", Value = " + userValue, True, True)
...
try:
    ...
    # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetDpn() Primary ID = " + productIdentification.PrimaryId, False, False)
    ...
    if keyValuePairs[0] != productIdentification.BuildData:
        for keyValuePair in keyValuePairs:
            ...
            if (key == "VIN"):
                ...
                MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetVinAndModelYear() VIN = " + value, False, False)
                break
    gmEdiBuildResultsRecord.HrSetTestPgmName(testAppResults.MainTestApplication.GmReporting.MersTestName)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetTestPgmName() value = " + testAppResults.MainTestApplication.GmReporting.MersTestName, False, False)
    gmEdiBuildResultsRecord.HrSetTestType(testAppResults.MainTestApplication.GmReporting.MersTestType)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetTestType() value = " + testAppResults.MainTestApplication.GmReporting.MersTestType, False, False)
    gmEdiBuildResultsRecord.HrSetTestPgmRev(testAppResults.MainTestApplication.GmReporting.MersTestPgmRev)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetTestPgmRev() value = " + str(testAppResults.MainTestApplication.GmReporting.MersTestPgmRev), False, False)
    gmEdiBuildResultsRecord.HrSetTestTableRev(testAppResults.MainTestApplication.GmReporting.MersTestTableRev)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetTestTableRev() value = " + str(testAppResults.MainTestApplication.GmReporting.MersTestTableRev), False, False)
    gmEdiBuildResultsRecord.HrSetTcaNumber(testAppResults.MainTestApplication.GmReporting.MersTca)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetTcaNumber() value = " + testAppResults.MainTestApplication.GmReporting.MersTca, False, False)
    gmEdiBuildResultsRecord.HrSetEngineType(testAppResults.MainTestApplication.GmReporting.MersEngineType)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetEngineType() value = " + testAppResults.MainTestApplication.GmReporting.MersEngineType, False, False)
    gmEdiBuildResultsRecord.HrSetDevIdBitMap(0, int(testAppResults.MainTestApplication.GmReporting.MersDeviceId, 0))
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetDevIdBitMap() value = " + testAppResults.MainTestApplication.GmReporting.MersDeviceId, False, False)
    gmEdiBuildResultsRecord.HrSetUserValue(1, testAppResults.MainTestApplication.TestCycleResults.DurationMsec / 1000)
    MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetUserValue() User Value Id = 1, Value = " + str(testAppResults.MainTestApplication.TestCycleResults.DurationMsec / 1000), False, False)
    ...
    if (overallTestResultsPass):
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetStaticPassFailFlag() value = P", False, False)
        gmEdiBuildResultsRecord.ScAddShipCode(testAppResults.MainTestApplication.GmReporting.GepicsShipCode, ord('P'))
        MainTPassScripting.InterfaceUiLogger("DEMO", "ScAddShipCode() value = P, ShipCode = " + testAppResults.MainTestApplication.GmReporting.GepicsShipCode, False, False)
    else:
        gmEdiBuildResultsRecord.HrSetStaticPassFailFlag(ord('F'))
        MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetStaticPassFailFlag() value = F", False, False)
        gmEdiBuildResultsRecord.ScAddShipCode(testAppResults.MainTestApplication.GmReporting.GepicsShipCode, ord('F'))
        MainTPassScripting.InterfaceUiLogger("DEMO", "ScAddShipCode() value = F, ShipCode = " + testAppResults.MainTestApplication.GmReporting.GepicsShipCode, False, False)

    if (testAppResults.MainTestApplication.TestCycleResults.Abort):
        #gmEdiBuildResultsRecord.HrSetAbortedFlag(ord('A'))
        #MainTPassScripting.InterfaceUiLogger("DEMO", "HrSetAbortedFlag() value = A", False, False)
    ...
    if (SystemConfigurationValue.PartnerInterfaces.GmEdi.CreateMersReportingRecord): 
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "HrCreateHistoryRecord()", False, False)
    else:
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "System Parameter 'Create Mers Reporting Record' = False, No Mers Reporting Record Created", False, False)

except Exception as inst:
    TPassLogger.Warn("Test Results Processing Script:  Error Creating MERS History Record.  Exception Occurred :{0}", inst)
    MainTPassScripting.InterfaceUiLogger("DEMO", "Error Creating MERS History Record.  Exception Occurred = " + str(inst), True, True)
    isSuccess = False
...
try:
    if (SystemConfigurationValue.PartnerInterfaces.GmEdi.CreateGepicsShipRecord): 
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "ScCreateShippingCtrlRecord()", False, False)
    else:
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "System Parameter 'Create Gepics Ship Record' = False, No Gepics Ship Record Created", False, False)

except Exception as inst:
    TPassLogger.Warn("Test Results Processing Script:  Error Creating GEPICS SHIPPING Record.  Exception Occurred :{0}", inst)
    MainTPassScripting.InterfaceUiLogger("DEMO", "Error Creating GEPICS SHIPPING Record.  Exception Occurred = " + str(inst), True, True)
    isSuccess = False
...
try:     
    if (SystemConfigurationValue.PartnerInterfaces.GmEdi.CreateGsipQualityRecord): 
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "GrCreateGsipRecord()", False, False)
    else:
        ...
        MainTPassScripting.InterfaceUiLogger("DEMO", "System Parameter Create Gsip Quality Record = False, No Gsip Quality Record Created", False, False)

except Exception as inst:
    TPassLogger.Warn("Test Results Processing Script:  Error Creating GSIP Quality Record.  Exception Occurred :{0}", inst)
    MainTPassScripting.InterfaceUiLogger("DEMO", "Error Creating GSIP Quality Record.  Exception Occurred = " + str(inst), True, True)
    isSuccess = False
