...
try:
    ...
    TPassLogger.Debug("Option Retrieval Script:  Return from GM EDI = {0}", refReturnValue.Value)
    if refReturnValue.Value == 0x40000:
        ...
        MainTPassScripting.InterfaceUiLogger("GM EDI", "Build Data FOUND for Primary Id = " + RunTestCycleId + ", Build Data = " + productBuildData, False, False)
        MainTPassScripting.InterfaceUiLogger("GM EDI", "Build Data Source = " + refBuildDataSource.Value, False, False)
        TPassLogger.Debug("Option Retrieval Script:  Product Build Data from GM EDI = {0}", productBuildData)
        TPassLogger.Debug("Option Retrieval Script:  Build Data Source from GM EDI = {0}", refBuildDataSource.Value)
        isSuccess = True
    else:
        # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
        MainTPassScripting.InterfaceUiLogger("GM EDI", "Build Data NOT FOUND for Primary Id = " + RunTestCycleId, True, False)
        TPassLogger.Debug("Option Retrieval Script:  Build Data NOT FOUND for Product Id = {0}", RunTestCycleId)
        isSuccess = False

except Exception as inst:
    # InterfaceUiLogger(string mesSystem, string detail, bool isError = false, bool alarm = false)
    MainTPassScripting.InterfaceUiLogger("GM EDI", "Get Build Data for Primary Id " + RunTestCycleId + ": " + str(inst), True, True)
