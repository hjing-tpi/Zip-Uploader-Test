...
def SetProcessData(reportId, reportData):
    ...
    if (reportId and reportData):
        # Saving Process Data 
        processResultsText = processResultsText + reportId + "=" + reportData + "\r\n"
...
try:
    ...
    # Remove Process Data file if exist
    if File.Exists(processDataFile + "TMP"):
        try:
            File.Delete(processDataFile + "TMP")
        except Exception as inst:
            TPassLogger.Debug("Results Processing Script:  File.Delete Exception Occurred :{0}", inst)            
    if File.Exists(processDataFile):
        try:
            File.Delete(processDataFile)
        except Exception as inst:
            TPassLogger.Debug("Results Processing Script:  File.Delete Exception Occurred :{0}", inst)            
    ...
    if Char.IsDigit(primaryPrefix) == False:
        processResultsText = productIdentification.PrimaryId.Remove(0,1) + "\r\n"
    else:
        processResultsText = productIdentification.PrimaryId + "\r\n"
        ...
        for subGroupInx in range(len(testAppResults.MainTestApplication.TestCycle[groupInx])):
            for testInx in range(len(testAppResults.MainTestApplication.TestCycle[groupInx][subGroupInx].TestSteps)):
                ...
                if (testStep.Name == "LinValidatePartNumber"):
                    if (str(testStep.TestStepResults.TestResults) != str(TestResults.NotTested) and str(testStep.TestStepResults.TestResults) != str(TestResults.OptionCodeNotTested)):
                        SetProcessData(testStep.LinValidatePartNumber.TestDataReporting.BroadcastedPartNumber, testStep.LinValidatePartNumber.TestData.BroadcastedPartNumber)
                        SetProcessData(testStep.LinValidatePartNumber.TestDataReporting.ProcessedPartNumber, testStep.LinValidatePartNumber.TestData.ProcessedPartNumber)
...
try:
    ...
    TPassLogger.Warn("Test Results Processing Script:  Error Creating Test Results.  Exception Occurred :{0}", inst)
    isSuccess = False

# Create Process Data file for Blade to process
try:
    try:
        File.WriteAllText(processDataFile + "TMP", processResultsText)
        File.Move(processDataFile + "TMP", processDataFile)
        MainTPassScripting.InterfaceUiLogger("Blade", "Process Data File Written - " + processDataFile, False, False)
    except Exception as inst:
        MainTPassScripting.InterfaceUiLogger("Blade", "File.WriteAllText Exception Occurred" + str(inst), True, True)
        TPassLogger.Error("Test Results Processing Script:  File Exception Occurred :{0}", inst)            

    TPassLogger.Debug("Test Results Processing Script:  Product Primary ID = {0}, Blade Process Data File Text = {1}", productIdentification.PrimaryId, processResultsText )

except Exception as inst:
    TPassLogger.Warn("Test Results Processing Script:  Error Creating Process Data Results.  Exception Occurred :{0}", inst)
    isSuccess = False
...
try:
   ...
   # Add test results
    ...
    try:
        ...
        try:
            while udpSendReceiveSuccess == False and plcUdpSendRetries <= plcUdpSendRetryLimit:
                ...
                if udpClient.AsyncSend(int(plcIp[0]), int(plcIp[1]), int(plcIp[2]), int(plcIp[3]), plcRemotePort, plcTestResultsBytes) == True:
                    ...
                    if str(udpClient.SendAsyncStage) == str(udpClient.SendAsyncStages.Success):
                        ...
                        #REMOVE next line if ACK logic is uncommented
...
#	Date: 07162024
#	Version: 5.0
#	Change: Added logic to sent Process Data to Blade system
#	Date: 05312024
#	Version: 4.0
#	Change: Fixed defects for sending test results to PLC via UDP
#	Date: 05222024
#	Version: 3.0
