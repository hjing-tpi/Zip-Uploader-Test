...
uncertifiedRPOS = False
#
#########################################################################################################################################
#########################################################################################################################################
# Software Engineer: This box is used for debugging purposes, comment out when done manually testing. 
# from System.Collections.Generic import List, Dictionary
# BuildData = """FLASHHOLD=P,STATICHOLD=P,PVIF=221000010,PVI=000010,VIN=1GT40FDA1NU000001,MDYR=N,CSN=1ZZ0100001 ,ASSM=1,PDYR=2022,MD=35743  ,RP01=MAO,RP02=,
    ...
    if keyValuePairs[0] != BuildData:
        ...
        if (OptionCodesInBuildData.Contains("EN0")):    #Engine none
            ...
            ## More Exports
                ...
                OptionCodesInBuildData.Add("EUDC")
                OptionCodesInBuildData.Add("EUAC")
            ## Japan And China, note Uncertified RPOs must be false
            elif not uncertifiedRPOS and OptionCodesInBuildData.Contains("MAS"):
            ...
            elif not uncertifiedRPOS and OptionCodesInBuildData.Contains("MAO"):
            ...
            ## Disables the Japan and China RPOs from being tested.
            elif uncertifiedRPOS and (OptionCodesInBuildData.Contains("MAO") or OptionCodesInBuildData.Contains("MAS")):
                TPassLogger.Info("Option Parsing Script: EN0 exists but Tester Not Certified for these RPO Codes: MAO, MAS DPN = " + primaryId)
                try:
                    AlertMessage = """
                    <fontsize:32> >: Option Parsing Script", The CPT tester is not certified for these RPO codes: MAO, MAS. Test Cycle will not run.  DPN = {primaryId} </fontsize>""".format(primaryId = primaryId)
                    MainTPassScripting.Alert("WARNING", AlertMessage, 32.0, Brushes.Red)
                except Exception as ex:
                    MainTPassScripting.InterfaceUiLogger("Option Parsing Script", "EN0 exists but Tester Not Certified for these RPO Codes: MAO, MAS DPN = " + primaryId, True, True)
                raise Exception("Uncertified RPO Codes in Build Data, isSuccess = False")
            else:
                isSuccess = True
