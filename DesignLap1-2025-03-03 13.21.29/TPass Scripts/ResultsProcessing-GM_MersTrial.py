...
try:
    ...
    gmEdiBuildResultsRecord.HrSetOptionString("B:12345678D:12345678")
    ...
    # Get VIN, CSN and MD from Build Data
    builddataCsn = ""
    builddataMd = ""
...
# Create Printed Label Text to pass back to TPass for printing.  For failed test, it is the same as the label that is going to be printed
try:
    DisplayedLabelText = ""
    if (overallTestResultsPass):
        DisplayedLabelText = "<fontsize:32>" + printedLabelText# + "<bold>\r\n\r\nPASSED\r\nPASSED\r\nPASSED\r\nPASSED" + "</bold></fontsize>"
    else:
        DisplayedLabelText = "<fontsize:32>" + printedLabelText + printedFaultText + "</fontsize>"
        #DisplayedLabelText = "<b>The reason</b> <fontsize:30>why this failed</fontsize> <fontsize:12><color:green>is because somebody turned</color> the light off... duh!</fontsize>"

    MainTPassScripting.UpdateTestResultsScreenCustomText(DisplayedLabelText)
    TPassLogger.Debug("Test Results Processing Script:  Product Primary ID = {0}, Screen Custom Label Text = {1}", productIdentification.PrimaryId, printedLabelText )
