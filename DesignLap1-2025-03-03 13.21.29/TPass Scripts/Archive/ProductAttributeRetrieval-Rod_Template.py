...
try:
    ...
    # Request build data from the plants MES system using the passed in "RunTestCycleId" 
    #MainTPassScripting.InterfaceUiLogger("MES", "Attribute Retrieval", True, True)
    if str(MainTPassScripting.StartTestCycleTriggerType) == str(MainTPassScripting.TriggerType.Scan):
        TPassLogger.Debug("OptionRetrieval - Trigger Type is Scan")
    elif str(MainTPassScripting.StartTestCycleTriggerType) == str(MainTPassScripting.TriggerType.ContinuousMode):
        TPassLogger.Debug("OptionRetrieval - Trigger Type is ContinuousMode")
