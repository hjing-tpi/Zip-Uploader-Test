#Create a Grupo Test Results Record and format and return the Printed Label Text for TPass to send to a printer
...
try:
    ...
    try:
        ...
        MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Test Results Processing Script:  Error Reading Test ID File, will default to Tester ID = 0.  Exception Occurred = " + str(inst), True, True)
    ...
    #ToDo:  When we switch to sending results to Blade Interface system, need to strip alpha characters off to ensure the PrimaryId is numeric
    ...
    # Set Results to send to Toluca SQL Interface
    ...
    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Error Creating Results Record.  Exception Occurred = " + str(inst), True, True)
    isSuccess = False

# Create Results file for Toluca SQL Interface to process
try:
    try:
        File.WriteAllText(resultsFile + "TMP", testResultsText)
        File.Move(resultsFile + "TMP", resultsFile)
        MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Results File Written - " + resultsFile, False, False)
    except Exception as inst:
