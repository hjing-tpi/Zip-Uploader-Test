...
try:
    ...
    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Trigger Type = " + str(MainTPassScripting.StartTestCycleTriggerType), False, False)
    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Data = " + RunTestCycleId, False, False)
    ...
    try:
        ...
        MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Option Retrieval Script:  Error Reading Test ID File, will default to Tester ID = 0.  Exception Occurred = " + str(inst), True, True)
    ...
    # If data didn't come from File drop, retrieve the build data using Toluca SQL Interface.  Otherwise data received is complete
    if str(MainTPassScripting.StartTestCycleTriggerType) != str(MainTPassScripting.TriggerType.FileDrop):
        ...
        # Create Request file for Toluca SQL Interface to read, query the plant database and write the results
        try:
            File.WriteAllText(requestFile + "TMP", testerId + "\r\n" + RunTestCycleId + "\r\n" + "\r\n")
            File.Move(requestFile + "TMP", requestFile)
            MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Request File Written - " + requestFile, False, False)
        except Exception as inst:
            MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "File.WriteAllText Exception Occurred" + str(inst), True, True)
        ...
        try:
            ...
            while (DateTime.Now - startDateTime).TotalMilliseconds < buildDataRequestTimeoutMsec and not buildDataFileFound:
                ...
                if File.Exists(buildDataFile):
                    ...
                    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Build Data File Received - " + buildDataFile, False, False)

                    #Read contents and delete file
                    productBuildData = File.ReadAllText(buildDataFile)
                    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Build Data Received - " + productBuildData, False, False)
                    isSuccess = True
            ...
            if buildDataFileFound == False:
                MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Build Data File Not Received - " + buildDataFile, True, False)
                productBuildData = ""
        
        except Exception as inst:
            MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Read Build Data File Exception Occurred" + str(inst), True, True)
            TPassLogger.Error("Option Retrieval Script:  Read Build Data File Exception Occurred :{0}", inst)            
        
    else:
        MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Build data complete no need to interface with Toluca SQL Interface", False, False)
        TPassLogger.Debug("Option Retrieval Script - Build data complete no need to interface with Toluca SQL Interface")
        productBuildData = RunTestCycleId
        isSuccess = True


except Exception as inst:
    MainTPassScripting.InterfaceUiLogger("Toluca SQL Interface", "Get Build Data Failed: " + str(inst), True, True)
