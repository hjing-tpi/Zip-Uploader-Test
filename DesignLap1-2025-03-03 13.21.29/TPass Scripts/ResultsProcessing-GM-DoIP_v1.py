...
def SetFault(faultId, faultDescription):
    ...
    global wirelessFaultOccurred
    global wirelessFaultId
    if (faultId):
        try:
            ...
            if (ediFaultId == wirelessFaultId):
                wirelessFaultOccurred = True
...
try:
    ...
    screenLabelText = screenLabelText + "STN:" + stationNumber + "    " + " DPN:" + productIdentification.PrimaryId + "    " + "CSN:<b>" + builddataCsn + "</b>    " + "MODEL:" + builddataMd + "\r\n"
    ...
    for groupInx in range(len(testAppResults.MainTestApplication.TestCycle)):
        for subGroupInx in range(len(testAppResults.MainTestApplication.TestCycle[groupInx])):
            for testInx in range(len(testAppResults.MainTestApplication.TestCycle[groupInx][subGroupInx].TestSteps)):
                ...
                if (testStep.Name == "DoIPReceiveValidate"):
                    for dataEntity in testStep.DoIPReceiveValidate.DoIPDataEntities:
                        if (IsTestFailed(str(dataEntity.TestStepResults.TestResults))):
                            SetFault(dataEntity.Fault.Id, dataEntity.Fault.Description)

                if (testStep.Name == "DoIPSendReceiveValidate"):
                    for dataEntity in testStep.DoIPSendReceiveValidate.DoIPDataEntities:
                        if (IsTestFailed(str(dataEntity.TestStepResults.TestResults))):
                            SetFault(dataEntity.Fault.Id, dataEntity.Fault.Description)

                if (testStep.Name == "DoIPValidateSavedDtcData"):
                    for dtc in testStep.DoIPValidateSavedDtcData.Dtcs:
                        if (IsTestFailed(str(dtc.TestStepResults.TestResults))):
                            SetFault(dtc.Fault.Id, dtc.Fault.Description)
                            if (dtc.Fault.Detail):
                                screenLabelFaultText = screenLabelFaultText + "   " + dtc.Fault.Detail + "\r\n"
                ...
                if (testStep.Name == "DoIPValidatePartNumber"):
                    if (str(testStep.TestStepResults.TestResults) != str(TestResults.NotTested) and str(testStep.TestStepResults.TestResults) != str(TestResults.OptionCodeNotTested)):
                        ...
                        if (str(testStep.TestStepResults.TestResults) != str(TestResults.Pass)):
                            ...
                            if (numberFaultsAdded < maxNumberFaultsOnPrintedLabel):
                                if (testStep.Fault.Detail):
                                    numberFaultsAdded = numberFaultsAdded + 1
                                    printedFaultText = printedFaultText + "   " + testStep.Fault.Detail + "\r\n"
                                    #printedFaultText = printedFaultText + "   PN from Broadcast: " + testStep.DoIPValidatePartNumber.TestData.BroadcastedPartNumber + "," + " PN from Module: " + testStep.DoIPValidatePartNumber.TestData.ProcessedPartNumber + "\r\n"

                        if (testStep.DoIPValidatePartNumber.TestDataReporting.BroadcastedPartNumber):
                            try:
                                userValueIds = testStep.DoIPValidatePartNumber.TestDataReporting.BroadcastedPartNumber.Split(',')
                                if len(userValueIds) != 2:
                                    TPassLogger.Error("Test Results Processing Script:  BroadcastedPartNumberId format must contain two IDs and comma delimited.  Id = {0}", testStep.DoIPValidatePartNumber.TestDataReporting.BroadcastedPartNumber)
                                else:
                                    try:
                                        userValues = testStep.DoIPValidatePartNumber.TestData.BroadcastedPartNumber
                                        if (not Regex.IsMatch(userValues, "^\d{8}$")):
                                            TPassLogger.Warn("Test Results Processing Script:  BroadcastedPartNumber format must be 8 digits.  Value = {0}", testStep.DoIPValidatePartNumber.TestData.BroadcastedPartNumber)
                                        else:
                                            userValues = [userValues[i:i+4] for i in range(0, len(userValues), 4)]
                                            SetProcessData(userValueIds[0], userValues[0])
                                            SetProcessData(userValueIds[1], userValues[1])
                                ...
                                TPassLogger.Warn("Test Results Processing Script:  Error Setting User Value in MERS History Record.  Exception Occurred :{0}", inst)
                                TPassLogger.Warn("Test Results Processing Script:  BroadcastedPartNumberId format must contain two IDs and comma delimited.  Id = {0}", testStep.DoIPValidatePartNumber.TestDataReporting.BroadcastedPartNumber)
 
                        if (testStep.DoIPValidatePartNumber.TestDataReporting.ProcessedPartNumber):
                            try:
                                userValueIds = testStep.DoIPValidatePartNumber.TestDataReporting.ProcessedPartNumber.Split(',')
                                if len(userValueIds) != 2:
                                    TPassLogger.Error("Test Results Processing Script:  ProcessedPartNumberId format must contain two IDs and comma delimited.  Id = {0}", testStep.DoIPValidatePartNumber.TestDataReporting.ProcessedPartNumber)
                                else:
                                    try:
                                        userValues = testStep.DoIPValidatePartNumber.TestData.ProcessedPartNumber
                                        if (not Regex.IsMatch(userValues, "^\d{8}$")):
                                            TPassLogger.Warn("Test Results Processing Script:  ProcessedPartNumber format must be 8 digits.  Value = {0}", testStep.DoIPValidatePartNumber.TestData.ProcessedPartNumber)
                                        else:
                                            userValues = [userValues[i:i+4] for i in range(0, len(userValues), 4)]
                                            SetProcessData(userValueIds[0], userValues[0])
                                            SetProcessData(userValueIds[1], userValues[1])
                                ...
                                TPassLogger.Warn("Test Results Processing Script:  Error Setting User Value in MERS History Record.  Exception Occurred :{0}", inst)
                                TPassLogger.Warn("Test Results Processing Script:  ProcessedPartNumberId format must contain two IDs and comma delimited.  Id = {0}", testStep.DoIPValidatePartNumber.TestDataReporting.ProcessedPartNumber)

    # Create MERS Record
    if (SystemConfigurationValue.PartnerInterfaces.GmEdi.CreateMersReportingRecord): 
        if (wirelessFaultOccurred == True):
            gmEdiBuildResultsRecord.HrClearFaults()
            gmEdiBuildResultsRecord.HrSetFault(wirelessFaultId)
            TPassLogger.Info("Test Results Processing Script:  Wireless Com Fault occurred, clearing all faults except Wireless Com Fault")
            MainTPassScripting.InterfaceUiLogger("GM EDI", "HrClearFaults() - Wireless Com Fault occurred, clearing all faults except Wireless Com Fault " + str(wirelessFaultId).zfill(5), False, False)
