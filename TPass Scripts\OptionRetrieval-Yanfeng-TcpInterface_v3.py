...
try:
    ...
    if date check fail:
        MainTPassScripting.InterfaceUiLogger("TPass", "License Expired on Tool", True, True)
        isSuccess = False
    else:
        
        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Trigger Type = " + str(MainTPassScripting.StartTestCycleTriggerType), False, False)
        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Data = " + RunTestCycleId, False, False)

        # If data didn't come from File drop or Scan, retrieve the build data using TCPInterface.  Otherwise data received is complete
        if str(MainTPassScripting.StartTestCycleTriggerType) != str(MainTPassScripting.TriggerType.FileDrop) and str(MainTPassScripting.StartTestCycleTriggerType) != str(MainTPassScripting.TriggerType.Scan):
            TPassLogger.Debug("Option Retrieval Script - Trigger Type is Manual retrieve build data")
            ...
            # Create Request file for TCPInterface to read, query the plant database and write the results
            try:
                File.WriteAllText(requestFile + "TMP", RunTestCycleId)
                File.Move(requestFile + "TMP", requestFile)
                MainTPassScripting.InterfaceUiLogger("TCPInterface", "Request File Written - " + requestFile, False, False)
            except Exception as inst:
                MainTPassScripting.InterfaceUiLogger("TCPInterface", "File.WriteAllText Exception Occurred" + str(inst), True, True)
            ...
            try:
                ...
                while (DateTime.Now - startDateTime).TotalMilliseconds < buildDataRequestTimeoutMsec and not buildDataFileFound:
                    ...
                    if File.Exists(buildDataFile):
                        ...
                        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Build Data File Received - " + buildDataFile, False, False)

                        #Read contents and delete file
                        productBuildData = File.ReadAllText(buildDataFile)
                        MainTPassScripting.InterfaceUiLogger("TCPInterface", "Build Data Received - " + productBuildData, False, False)
                        isSuccess = True
                ...
                if buildDataFileFound == False:
                    MainTPassScripting.InterfaceUiLogger("TCPInterface", "Build Data File Not Received - " + buildDataFile, True, False)
                    productBuildData = ""
            
            except Exception as inst:
                MainTPassScripting.InterfaceUiLogger("TCPInterface", "Read Build Data File Exception Occurred" + str(inst), True, True)
                TPassLogger.Error("Option Retrieval Script:  Read Build Data File Exception Occurred :{0}", inst)            
            
        else:
            MainTPassScripting.InterfaceUiLogger("TCPInterface", "Build data complete no need to interface with TCPInterface", False, False)
            TPassLogger.Debug("Option Retrieval Script - Build data complete no need to interface with TCPInterface")
            productBuildData = RunTestCycleId
            isSuccess = True


except Exception as inst:
    MainTPassScripting.InterfaceUiLogger("TCPInterface", "Get Build Data Failed: " + str(inst), True, True)
...
#   Date: 09202022
#   Version: 2.0
#   Change: Create temporary request.txt file then rename, to avoid file contention issues with TCP Interface
#   Date: 06222022
#   Version: 1.0
